...
🚀 [1/9] Start a local 3-node CockroachDB cluster...
...
🚀 [2/9] Set up the demo schema with comments on database objects...
...
🚀 [3/9] Save the comments to a CSV file...
...
🚀 [4/9] Create the golden master backup...
...
----------------------+-----------+--------------------+-------
  1189397007112994817 | succeeded |                  1 |    2
(1 row)
...
🚀 [5/9] Drop the golden master database...
...
🚀 [6/9] Restore the golden master backup to a new database...
...
---Running
        job_id        |  status   | fraction_completed | rows
----------------------+-----------+--------------------+-------
  1189397038275297281 | succeeded |                  1 |    2
(1 row)
...
🚀 [7/9] Make the saved comments available for IMPORT...
...
🚀 [8/9] Import the saved comments to a working table...
...
----------------------+-----------+--------------------+------+---------------+--------
  1189397089212366849 | succeeded |                  1 |    6 |             6 |  1941
(1 row)
...
🛠️  Show the updated working table with corrected database name
...
-[ RECORD 1 ]
comment_type           | 0
comment_type_name      | Database
object_database_name   | newdb
object_schema_name     | NULL
object_name            | newdb
object_sub_name        | NULL
table_name             | NULL
column_name            | NULL
index_name             | NULL
schema_name            | NULL
...
comment_text           | (database) Scratch database used to demonstrate all eight CockroachDB comment types.
-[ RECORD 2 ]
comment_type           | 1
comment_type_name      | Table/View/Sequence
object_database_name   | newdb
object_schema_name     | inventory
...
function_name          | NULL
user_defined_type_name | NULL
comment_text           | (table) Master catalogue of every product stocked in the warehouse; one row per distinct SKU.
-[ RECORD 3 ]
comment_type           | 2
comment_type_name      | Column
object_database_name   | newdb
object_schema_name     | inventory
object_name            | products
object_sub_name        | sku
...
function_name          | NULL
user_defined_type_name | NULL
comment_text           | (column) Supplier-assigned stock-keeping unit code.  Must be globally unique across all suppliers.
-[ RECORD 4 ]
comment_type           | 3
comment_type_name      | Index
object_database_name   | newdb
object_schema_name     | inventory
object_name            | products
object_sub_name        | idx_products_name
table_name             | NULL
column_name            | NULL
...
function_name          | NULL
user_defined_type_name | NULL
comment_text           | (index) Supports fast product-name searches and ORDER BY name queries across the full catalogue.
-[ RECORD 5 ]
comment_type           | 4
comment_type_name      | Schema
object_database_name   | newdb
object_schema_name     | inventory
...
function_name          | NULL
user_defined_type_name | NULL
comment_text           | (schema) Contains all tables, types, functions, and supporting objects for warehouse inventory management.
-[ RECORD 6 ]
comment_type           | 5
comment_type_name      | Constraint
object_database_name   | newdb
object_schema_name     | inventory
object_name            | products
object_sub_name        | chk_positive_price
table_name             | NULL
column_name            | NULL
index_name             | NULL
schema_name            | NULL
...
function_name          | NULL
user_defined_type_name | NULL
comment_text           | (constraint) Every stocked item must carry a positive list price; complimentary items are handled through a separate promotions workflow, not a zero price.
...
🚀 [9/9] Restore the comments to the database objects...
